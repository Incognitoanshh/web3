...
export default function Register(props) {
  ...
  const [privateKeyValue, setPrivateKeyValue] = useState("");
  const [privateKeyRetValue, setPrivateKeyRetValue] = useState("");
  const [mnemonicInput, setMnemonicInput] = useState(""); // ✅ Using state for mnemonic input
  const [retrieve, setRetrieve] = useState(false);
  ...
  function retrieveIdentity() {
    ...
    if (!ethers.utils.isValidMnemonic(mnemonicInput)) {
      alert("Invalid mnemonic phrase! Please enter a valid 12-word phrase.");
      return;
    }
      ...
      {props.showIdentity && (
        <div id="registerOptions">
          ...
          <div>
            ...
            <input
              type="text"
              value={mnemonicInput} // ✅ Controlled input
